...
function unitData = processUnit(unitData, baselineWindow, postWindow, SCALING_FACTOR)
    ...
    % Calculate baseline median (only for non-zero values)
    psth = unitData.psthSmoothed;
    baselineMedian = median(psth(baselineIdx & psth > 0), 'omitnan');
    
    % Calculate percent change
    if isnan(baselineMedian) || baselineMedian == 0
        baselineMedian = SCALING_FACTOR;
        psthPercentChange = (((psth + SCALING_FACTOR) - SCALING_FACTOR) / SCALING_FACTOR) * 100;
    else
        psthPercentChange = ((psth - baselineMedian) / baselineMedian) * 100;
    end
    ...
    stats.baseline.median = baselineMedian;
