...
if nargin>6 && ~isempty(varargin{5})
    ...
    if nargin > 2 && ~isempty(varargin{1}), pre_time = varargin{1}; else, pre_time = 1; end
    if nargin > 3 && ~isempty(varargin{2}), post_time = varargin{2}; else, post_time = 1; end
    if nargin > 4 && ~isempty(varargin{3}), bw = varargin{3}; else, bw = 10; end  % Bin width in ms

    % Compute bin edges
    lastBin = ceil(post_time * 1000);  % Last bin edge in ms
    edge = edgeCalculator(bw, -pre_time * 1000, lastBin);  % Corrected function call
...
%% Helper Function: Edge Calculator
function edges = edgeCalculator(binWidth, start, stop)
    % Generates edges for histogram binning
    if start >= stop
