function plotTimeLockedMeanPSTHCombined(cellDataStruct, figureFolder, treatmentTime, plotType, unitFilter)
    ...
    for g = 1:length(groupNames)
        ...
        for r = 1:length(recordings)
            ...
            decreasedPSTHs = [];
            noChangePSTHs = [];
            decreasedUnitIDs = {}; % Track IDs of decreased units for outlier analysis
                ...
                if isfield(unitData, 'psthSmoothed') && isfield(unitData, 'responseType')
                    ...
                    switch unitData.responseType
                        ...
                        case 'Decreased'
                            ...
                            decreasedUnitIDs{end + 1} = unitID; % Store unit ID for analysis
                        case 'No Change'
            ...
            % --- Identify top contributors in the Decreased group ---
            if ~isempty(decreasedPSTHs)
                ...
                % Sort the units by maximum firing rate in descending order
                [sortedMaxRates, sortIndices] = sort(maxFiringRates, 'descend');
                
                % Display the top units (e.g., top 2) contributing to the highest firing rates
                numTopUnits = 2;  % Number of top outlier units you want to identify
                fprintf('Top %d units contributing to the highest firing rates in "Decreased" plot:\n', numTopUnits);
                for i = 1:numTopUnits
                    unitIdx = sortIndices(i);  % Index of the unit in decreasedPSTHs
                    maxRate = sortedMaxRates(i);
                    
                    % Find the corresponding unit ID in decreasedUnitIDs
                    unitID = decreasedUnitIDs{unitIdx};  % Get unit ID from tracked list
                    
                    % Display unit ID and maximum firing rate
                    fprintf('Unit ID: %s, Max Firing Rate: %.2f spikes/s\n', unitID, maxRate);
                end
            end
            % -------------------------------------------------------
