function plotAllSmoothedPSTHs(cellDataStruct, treatmentTime, figureFolder)
    % Set defaults
        ...
        for r = 1:length(recordings)
            ...
            for u = 1:length(units)
                ...
                try
                    ...
                    if generatePlot(unitData, unitID, groupName, recordingName, treatmentTime, saveDir)
...
function success = generatePlot(unitData, unitID, groupName, recordingName, treatmentTime, saveDir)
    try
        ...
        title(sprintf('Smoothed PSTH: %s - %s - %s', groupName, recordingName, unitID));
        
        % Set limits
        maxY = max(unitData.psthSmoothed(~isnan(unitData.psthSmoothed)));
        ...
        fileName = sprintf('SmoothedPSTH_%s_%s.fig', unitID, timestamp);
...
function addMetadata(unitData, unitID)
    ...
    ann = annotation('textbox', [0.7, 0.8, 0.25, 0.15], ...
                    'String', metaText, ...
                    'EdgeColor', 'k', ...
                    'BackgroundColor', 'w', ...
...
function startDragFcn(src, ~)
    ...
    fig = gcf();
    set(fig, 'WindowButtonMotionFcn', {@dragFcn, src});
    set(fig, 'WindowButtonUpFcn', {@stopDragFcn, src});
end
