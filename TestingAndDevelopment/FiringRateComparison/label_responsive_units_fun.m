...
function responseTypeVec = categorize_units(FRs_before, FRs_after)
    ...
    for i = 1:length(FRs_before)
        ...
        % Generate bootstrapped differences with more meaningful noise
        boot_diffs = nan(nBootstraps, 1);
        for b = 1:nBootstraps
            resample_before = FRs_before(i) + randn * std(FRs_before);
            resample_after = FRs_after(i) + randn * std(FRs_after);
